  ...
  const cpp_typecheck_fargst &fargs)
{
  ...
  if(expr.type().id()==ID_code)
  {
    ...
    symbolt &func_symb=symbol_table.get(component_name);
  ...
  if(expr.type().id()==ID_constructor)
  {
    ...
    // will be available
    {
          ...
          it++)
      {
        ...
        {
          symbolt &s=symbol_table.get(it->get(ID_name));
          add_method_body(&s);
          break;
        }
      }
  ...
  side_effect_expr_function_callt &expr)
{
  ...
  add_method_body(&(symbol_table.get(symbol.name)));
...
void cpp_typecheckt::typecheck_expr_function_identifier(exprt &expr)
{
  ...
  {
    // Check if the function body has to be typechecked
    symbolt &func_symb=symbol_table.get(expr.get(ID_identifier));
