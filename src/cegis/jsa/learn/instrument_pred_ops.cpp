...
bool is_pred_op_decl(const namespacet &ns, const goto_programt::targett &pos)
{
  ...
  return type_eq(lhs, ns.lookup(id).type, ns);
}
}
...
goto_programt::targetst collect_pred_ops(jsa_programt &prog)
{
  ...
  for (goto_programt::targett it=body.begin(); it != end; ++it)
    if(is_pred_op_decl(ns, it))
      pred_ops.push_back(it);
  return pred_ops;
}
    ...
    pred_op_idst &op_ids, pred_op_idst &const_op_ids)
{
  ...
  const symbol_exprt pred_ops(ns.lookup(PRED_OPS).symbol_expr());
  const symbol_exprt pred_res_ops(ns.lookup(JSA_PRED_RES_OPS).symbol_expr());
  ...
  for (const goto_programt::targett &op : ops)
  {
    const symbol_exprt var(
      ns.lookup(get_affected_variable(*op)).symbol_expr());
    ...
    goto_programt::targett pos=jsa_assign(ns, gf, op, op_elem, var_ptr);
    if (!is_jsa_const(var))
    {
      ...
      pos=jsa_assign(ns, gf, pos, res_op_elem, address_of_exprt(var));
    }
    if (op == prog.synthetic_variables) prog.synthetic_variables=pos;
  }
  const symbol_exprt op_count(ns.lookup(JSA_PRED_OP_COUNT).symbol_expr());
  const constant_exprt op_value(from_integer(op_index, op_count.type()));
  goto_programt::targett &pos=prog.synthetic_variables;
  pos=jsa_assign(ns, gf, pos, op_count, op_value);
  const symbol_exprt res_cnt(ns.lookup(JSA_PRED_RESULT_OP_COUNT).symbol_expr());
  const constant_exprt res_value(from_integer(res_op_idx, res_cnt.type()));
  pos=jsa_assign(ns, gf, pos, res_cnt, res_value);
}
