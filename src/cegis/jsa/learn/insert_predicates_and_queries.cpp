    ...
    const size_t max_size)
{
  ...
  const symbol_exprt sz_expr(ns.lookup(size_id).symbol_expr());
...
void declare_jsa_predicates(jsa_programt &prog, const size_t max_sz)
{
  ...
  goto_functionst &gf=prog.gf;
  goto_programt &body=get_entry_body(gf);
  const symbol_exprt preds(ns.lookup(JSA_PREDS).symbol_expr());
  const symbol_exprt pred_sizes(ns.lookup(PRED_SIZES).symbol_expr());
  ...
  const size_t max_pred_size=get_max_pred_size(ns);
  for (mp_integer::ullong_t i=0; i < num_preds; ++i)
  {
    ...
    const symbol_exprt &local_pred(ns.lookup(local_pred_name).symbol_expr());
    const index_exprt local_preds_elem(local_pred, zero);
    pos=
      jsa_assign(ns, gf, pos, preds_elem, address_of_exprt(local_preds_elem));
    const index_exprt pred_sizes_elem(pred_sizes, index);
    const symbolt &sz_symb(ns.lookup(get_cegis_meta_name(sz_name)));
    pos=jsa_assign(ns, gf, pos, pred_sizes_elem, sz_symb.symbol_expr());
    pos=body.insert_after(pos);
    pos->type=goto_program_instruction_typet::FUNCTION_CALL;
    pos->source_location=jsa_builtin_source_location();
    code_function_callt call;
    call.function()=ns.lookup(JSA_ASSUME_VALID_PRED).symbol_expr();
...
{
  const namespacet ns(prog.st);
  declare_size_and_prog(
    prog, JSA_QUERY, JSA_QUERY_SZ, jsa_query_type, get_max_query_size(ns));
}
