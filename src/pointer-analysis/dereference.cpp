...
Function: build_byte_extract

  Inputs:

 Outputs:

 Purpose: Build byte_extract_* for the given root object and offset
...
Function: is_generic_pointer_type

  Inputs:

 Outputs:

 Purpose: Test whether type is char*, void*, or an integer type
...
{
  return
    base_type_eq(type, pointer_typet(empty_typet()), ns) ||
    base_type_eq(type, pointer_typet(char_type()), ns) ||
...
Function: has_pointer
 ...
 Purpose: Returns true, iff some operand has pointer type
  ...
  exprt &invalid_cond)
{
  ...
  #ifdef DEBUG
  std::cout << "DEREF: " << from_expr(ns, "", simp_pointer) << std::endl;
  #endif
...
Function: dereferencet::dereference_if
 ...
 Purpose: Compute *(c ? x : y) via c ? *x : *y
...
Function: dereferencet::dereference_plus
 ...
 Purpose: Compute *(p + c) for a pointer p and an integer c
  ...
  exprt &invalid_cond)
{
  if(expr.operands().size()>2)
    return dereference_rec(make_binary(expr), offset, type, invalid_cond);
  ...
  if(ns.follow(pointer.type()).id()==ID_pointer)
  {
    ...
    if(object_type.id()!=ID_empty) size=size_of_expr(object_type, ns);
  ...
  exprt &invalid_cond)
{
     ...
     expr_type.id()==ID_pointer)
  {
    ...
    if(is_generic_pointer_type(ns, expr_type) &&
       ...
       !is_generic_pointer_type(ns, pointer_typet(type)))
